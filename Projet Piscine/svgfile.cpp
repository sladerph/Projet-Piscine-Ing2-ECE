...
void Svgfile::addText(double x, double y, std::string text, std::string color, std::string align)
{
    /// <text x="180" y="60">Un texte</text>
    m_ostrm << "<text "
            << attrib("text-anchor", align)
            << attrib("x", x)
            << attrib("y", y)
            << attrib("fill", color);

    if (align == "middle")
        m_ostrm << attrib("dominant-baseline", "central");

    m_ostrm << ">" << text << "</text>\n";
}

void Svgfile::addText(double x, double y, double val, std::string color, std::string align)
{
    std::ostringstream oss;
    oss << val;
    addText(x, y, oss.str(), color, align);
}
